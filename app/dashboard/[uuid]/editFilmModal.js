...
export default function EditFilmModal({ film, isOpen, onClose, onRefresh }) {
  ...
  useEffect(() => {
    if (film) {
      setFormData({
        judul: film.judul || "",
        genre: film.genre_id || film.genre || "",
        sinopsis: film.sinopsis || "",
        trailer: film.trailer || "",
        poster: film.poster || "",
        kategori: film.kategori_id || film.kategori || "",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/film/update?id=${film.id}`, {
        method: "PUT",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#1a1f2e] border border-gray-700 w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-700 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white">Edit Detail Film</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-gray-400 uppercase">Genre (ID/Nama)</label>
        ...
        <div className="space-y-1">
            <label className="text-xs font-semibold text-gray-400 uppercase">Poster</label>
